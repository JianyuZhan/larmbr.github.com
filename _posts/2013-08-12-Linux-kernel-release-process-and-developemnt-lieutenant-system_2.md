...
3. **正式版本发布后的维护**
   ...
   这个**-stable分支**的起缘上一篇文章已经解释过, 它的目标就是: 从后续的版本中, 把修补bug或安全漏洞的补丁, **向后移植**到这个刚发布的版本中(当然, 这些修补肯定也会进入**mainline**中), 然后不定期地发布一个v2.6.X.Y(或v3.X.Y)的版本。随着每个Y版本号递增, 都会有若干个bug或漏洞被修补。
